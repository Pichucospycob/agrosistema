import require$$1$3, { app, BrowserWindow, dialog, ipcMain, Menu } from "electron";
...
function requireSymlink() {
  ...
  function createSymlinkSync(srcpath, dstpath, type2) {
    ...
    if (exists2) return fs2.symlinkSync(srcpath, dstpath, type2);
    mkdirsSync(dir);
...
function requireGt() {
  ...
  hasRequiredGt = 1;
  const compare = requireCompare();
  const gt2 = (a, b, loose) => compare(a, b, loose) > 0;
  gt_1 = gt2;
  return gt_1;
}
var lt_1;
...
function requireLt() {
  ...
  hasRequiredLt = 1;
  const compare = requireCompare();
  const lt2 = (a, b, loose) => compare(a, b, loose) < 0;
  lt_1 = lt2;
  return lt_1;
}
var eq_1;
...
function requireGte() {
  ...
  hasRequiredGte = 1;
  const compare = requireCompare();
  const gte2 = (a, b, loose) => compare(a, b, loose) >= 0;
  gte_1 = gte2;
  return gte_1;
}
var lte_1;
...
function requireLte() {
  ...
  hasRequiredLte = 1;
  const compare = requireCompare();
  const lte2 = (a, b, loose) => compare(a, b, loose) <= 0;
  lte_1 = lte2;
  return lte_1;
}
var cmp_1;
...
function drizzle(client, config = {}) {
  ...
  const session = new SQLJsSession(client, dialect, schema2, { logger });
...
const orders = sqliteTable("orders", {
  ...
  labor: text("labor"),
  // Labor
  implanted: integer("implanted", { mode: "boolean" }),
  // Implantado S/N
  ...
  // Pastilla
  nozzleDescription: text("nozzle_description"),
  // Ej: Abanico plano martillo
  waterPerHa: real("water_per_ha"),
  // Lts agua
  pressure: real("pressure"),
  // Presion (guardamos valor genérico o string si usa varias unidades)
  pressureUnit: text("pressure_unit"),
  // Bares, PSI, etc.
...
const orderLots = sqliteTable("order_lots", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  orderId: integer("order_id").references(() => orders.id).notNull(),
  lotId: integer("lot_id").references(() => lots.id).notNull(),
  appliedSurface: real("applied_surface").notNull()
...
const orderItems = sqliteTable("order_items", {
  ...
  // Dosis por ha
  quantityTheoretical: real("quantity_theoretical").notNull(),
  // Dosis Total (Teorico)
  // Remito / Devolución
  quantityDelivered: real("quantity_delivered").default(0),
  // Entreg. [lts]
  quantityReturned: real("quantity_returned").default(0),
  // Devuelto [lts]
  quantityReal: real("quantity_real").default(0)
  // Consumo real final
});
...
const emptyContainers = sqliteTable("empty_containers", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  productId: integer("product_id").references(() => products.id).notNull(),
  pendingReturn: integer("pending_return").default(0).notNull(),
  // En el campo/galpón
  deliveredCat: integer("delivered_cat").default(0).notNull()
  // Entregados al CAT
});
const supplierRemitos = sqliteTable("supplier_remitos", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  remitoNumber: text("remito_number").notNull(),
  // Número del remito del proveedor
  date: text("date").notNull(),
  supplier: text("supplier").notNull(),
  observations: text("observations"),
  createdAt: text("created_at").default(sql`CURRENT_TIMESTAMP`)
});
const supplierRemitoItems = sqliteTable("supplier_remito_items", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
function setupIpcHandlers(db, save) {
  ...
  ipcMain.handle("add-stock-movement", async (_event, { productId, quantity, description, type: type2 }) => {
    try {
      const result = await db.transaction(async (tx) => {
        ...
        await tx.update(products).set({ currentStock: newStock }).where(eq(products.id, productId));
  ...
  ipcMain.handle("get-orders", async () => {
    try {
      const orders$1 = await db.select().from(orders).orderBy(orders.date);
      const result = [];
      for (const order of orders$1) {
        const orderLots$1 = await db.select({
          name: lots.name
        }).from(orderLots).leftJoin(lots, eq(orderLots.lotId, lots.id)).where(eq(orderLots.orderId, order.id));
        const lotNames = orderLots$1.map((l) => l.name).join(", ");
  ...
  ipcMain.handle("create-order", async (_event, orderData) => {
    try {
      const result = await db.transaction(async (tx) => {
        const lastOrder = await tx.select().from(orders).orderBy(sql`${orders.orderNumber} DESC`).limit(1).get();
  ...
  ipcMain.handle("get-order-details", async (_event, orderId) => {
    ...
    const items = await db.select({
      ...
      dose: orderItems.dose,
      quantityTheoretical: orderItems.quantityTheoretical,
      quantityDelivered: orderItems.quantityDelivered,
      quantityReturned: orderItems.quantityReturned,
      quantityReal: orderItems.quantityReal
    }).from(orderItems).leftJoin(products, eq(orderItems.productId, products.id)).where(eq(orderItems.orderId, orderId));
    const lots$1 = await db.select({
      id: lots.id,
      name: lots.name,
      appliedSurface: orderLots.appliedSurface
    }).from(orderLots).leftJoin(lots, eq(orderLots.lotId, lots.id)).where(eq(orderLots.orderId, orderId));
    return { ...order, items, lots: lots$1 };
  });
  ipcMain.handle("emit-remito", async (_event, { orderId, items }) => {
    await db.transaction(async (tx) => {
      ...
      if (!remitoNum) {
        const lastRemitoOrder = await tx.select().from(orders).where(sql`${orders.remitoNumber} IS NOT NULL`).orderBy(sql`${orders.remitoNumber} DESC`).limit(1).get();
      ...
      for (const item of items) {
        if (item.id) {
          await tx.update(orderItems).set({ quantityDelivered: item.quantityDelivered }).where(eq(orderItems.id, item.id));
        }
        if (item.productId) {
          await tx.insert(stockMovements).values({
            productId: item.productId,
          ...
          if (product) {
            await tx.update(products).set({ currentStock: (product.currentStock || 0) - item.quantityDelivered }).where(eq(products.id, item.productId));
          }
        } else {
  ...
  ipcMain.handle("close-order", async (_event, { orderId, items }) => {
    await db.transaction(async (tx) => {
      await tx.update(orders).set({ status: "CERRADA" }).where(eq(orders.id, orderId));
      const order = await tx.select().from(orders).where(eq(orders.id, orderId)).get();
      for (const item of items) {
        const returned = item.quantityReturned || 0;
        const existingItem = await tx.select().from(orderItems).where(eq(orderItems.id, item.id)).get();
        ...
        if (returned > 0) {
          ...
          const product = await tx.select().from(products).where(eq(products.id, item.productId)).get();
          if (product) {
            await tx.update(products).set({ currentStock: product.currentStock + returned }).where(eq(products.id, item.productId));
          }
        }
      }
    });
  ...
  ipcMain.handle("get-stock-movements", async () => {
    try {
      ...
      }).from(stockMovements).leftJoin(products, eq(stockMovements.productId, products.id)).orderBy(stockMovements.date);
      return result;
    } catch (error2) {
  ...
  ipcMain.handle("get-consumption-by-campaign", async () => {
    try {
      const orders$1 = await db.select({
        id: orders.id,
        campaign: orders.campaign
      }).from(orders).where(eq(orders.status, "CERRADA"));
      const campaignIds = orders$1.map((o) => o.id);
      ...
      const items = await db.select({
        ...
        campaign: orders.campaign,
        date: orders.date
      }).from(orderItems).leftJoin(orders, eq(orderItems.orderId, orders.id)).leftJoin(products, eq(orderItems.productId, products.id)).where(eq(orders.status, "CERRADA"));
      return items;
    } catch (error2) {
      console.error(error2);
      throw error2;
  ...
  ipcMain.handle("get-container-status", async () => {
    try {
      const result = await db.select({
        ...
        pendingReturn: emptyContainers.pendingReturn,
        deliveredCat: emptyContainers.deliveredCat
      }).from(emptyContainers).leftJoin(products, eq(emptyContainers.productId, products.id));
      return result;
    } catch (error2) {
      console.error(error2);
      throw error2;
  ...
  ipcMain.handle("update-lot", async (_event, data) => {
    try {
  ...
  ipcMain.handle("get-remitos", async () => {
    try {
      return await db.select().from(remitos).orderBy(sql`${remitos.remitoNumber} DESC`);
    } catch (error2) {
      console.error(error2);
      throw error2;
    }
  });
  ipcMain.handle("get-aggregated-items-for-orders", async (_event, orderIds) => {
    try {
      const rawItems = await db.select({
        ...
        productPresentation: products.presentation,
        quantityTheoretical: orderItems.quantityTheoretical
      }).from(orderItems).leftJoin(products, eq(orderItems.productId, products.id)).where(sql`${orderItems.orderId} IN (${sql.join(orderIds, sql`, `)})`);
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        for (const dItem of deliveredItems) {
          ...
          const items = await tx.select().from(orderItems).where(sql`${orderItems.orderId} IN (${sql.join(orderIds, sql`, `)}) AND ${orderItems.productId} = ${dItem.productId}`);
          for (let i = 0; i < items.length; i++) {
            ...
            await tx.update(orderItems).set({ quantityDelivered: assign }).where(eq(orderItems.id, item.id));
          ...
          const product = await tx.select().from(products).where(eq(products.id, dItem.productId)).get();
          if (product) {
            await tx.update(products).set({ currentStock: (product.currentStock || 0) - dItem.quantityDelivered }).where(eq(products.id, dItem.productId));
          }
        }
        return newRemito;
      });
  ...
  ipcMain.handle("get-remito-details", async (_event, remitoId) => {
    try {
      const remito = await db.select().from(remitos).where(eq(remitos.id, remitoId)).get();
      if (!remito) throw new Error("Remito not found");
      const orders$1 = await db.select().from(orders).where(eq(orders.remitoId, remitoId));
      const rawItems = await db.select({
        productId: orderItems.productId,
        productName: products.name,
        productPresentation: products.presentation,
        ...
        quantityReturned: orderItems.quantityReturned,
        quantityReal: orderItems.quantityReal
      }).from(orderItems).innerJoin(orders, eq(orderItems.orderId, orders.id)).leftJoin(products, eq(orderItems.productId, products.id)).where(eq(orders.remitoId, remitoId));
      const aggregatedMap = /* @__PURE__ */ new Map();
      rawItems.forEach((item) => {
        if (!aggregatedMap.has(item.productId)) {
  ...
  ipcMain.handle("close-consolidated-remito", async (_event, { remitoId, itemsReturn, itemsDelivered }) => {
    try {
      const result = await db.transaction(async (tx) => {
        await tx.update(remitos).set({ status: "CERRADO" }).where(eq(remitos.id, remitoId));
        const remito = await tx.select().from(remitos).where(eq(remitos.id, remitoId)).get();
        await tx.update(orders).set({ status: "CERRADA" }).where(eq(orders.remitoId, remitoId));
        const linkedOrders = await tx.select().from(orders).where(eq(orders.remitoId, remitoId));
        if (itemsDelivered) {
          for (const dItem of itemsDelivered) {
            const currentItems = await tx.select().from(orderItems).where(sql`${orderItems.orderId} IN (${sql.join(linkedOrders.map((o) => o.id), sql`, `)}) AND ${orderItems.productId} = ${dItem.productId}`);
            ...
            if (Math.abs(diff) > 1e-4) {
              const product = await tx.select().from(products).where(eq(products.id, dItem.productId)).get();
              if (product) {
                await tx.update(products).set({ currentStock: (product.currentStock || 0) - diff }).where(eq(products.id, dItem.productId));
              }
              await tx.insert(stockMovements).values({
                productId: dItem.productId,
                quantity: -diff,
        ...
        for (const itemRet of itemsReturn) {
          if (itemRet.quantityReturned > 0) {
            ...
            const product = await tx.select().from(products).where(eq(products.id, itemRet.productId)).get();
            if (product) {
              await tx.update(products).set({ currentStock: (product.currentStock || 0) + itemRet.quantityReturned }).where(eq(products.id, itemRet.productId));
            }
            let remainingToReturn = itemRet.quantityReturned;
            for (const order of linkedOrders) {
              const orderItem = await tx.select().from(orderItems).where(sql`${orderItems.orderId} = ${order.id} AND ${orderItems.productId} = ${itemRet.productId}`).get();
              if (orderItem) {
                const currentDelivered = (await tx.select().from(orderItems).where(eq(orderItems.id, orderItem.id)).get())?.quantityDelivered || 0;
            ...
            for (const order of linkedOrders) {
              const orderItem = await tx.select().from(orderItems).where(sql`${orderItems.orderId} = ${order.id} AND ${orderItems.productId} = ${itemRet.productId}`).get();
              if (orderItem) {
                await tx.update(orderItems).set({ quantityReal: orderItem.quantityDelivered }).where(eq(orderItems.id, orderItem.id));
              }
            }
          }
        }
  ...
  ipcMain.handle("get-efficiency-report", async () => {
    try {
      const result = await db.select({
        ...
        field: orders.field,
        productName: products.name,
        theoretical: orderItems.quantityTheoretical,
        real: orderItems.quantityReal,
        campaign: orders.campaign,
        date: orders.date
      }).from(orderItems).leftJoin(orders, eq(orderItems.orderId, orders.id)).leftJoin(products, eq(orderItems.productId, products.id)).where(eq(orders.status, "CERRADA"));
      return result;
  ...
  ipcMain.handle("get-campaign-cost-report", async () => {
    try {
      const purchases = await db.select({
        productId: stockMovements.productId,
        productName: products.name,
        totalPurchased: sql`sum(${stockMovements.quantity})`
      }).from(stockMovements).leftJoin(products, eq(stockMovements.productId, products.id)).where(eq(stockMovements.type, "COMPRA")).groupBy(stockMovements.productId);
      const consumption = await db.select({
        productId: orderItems.productId,
        totalConsumed: sql`sum(${orderItems.quantityReal})`
      }).from(orderItems).leftJoin(orders, eq(orderItems.orderId, orders.id)).where(eq(orders.status, "CERRADA")).groupBy(orderItems.productId);
  ...
  ipcMain.handle("get-supplier-remitos", async () => {
    try {
      const result = await db.select().from(supplierRemitos).orderBy(sql`${supplierRemitos.createdAt} DESC`);
      return result;
    } catch (error2) {
      console.error(error2);
      throw error2;
  ...
  ipcMain.handle("create-supplier-remito", async (_event, data) => {
    try {
      const result = await db.transaction(async (tx) => {
        const [newRemito] = await tx.insert(supplierRemitos).values({
        ...
        for (const item of data.items) {
          await tx.insert(supplierRemitoItems).values({
            remitoId: newRemito.id,
            productId: item.productId,
          ...
          const product = await tx.select().from(products).where(eq(products.id, item.productId)).get();
          if (product) {
            await tx.update(products).set({ currentStock: (product.currentStock || 0) + item.quantity }).where(eq(products.id, item.productId));
          }
        }
        return newRemito;
      });
      ...
      console.error("Error creating supplier remito:", error2);
      throw error2;
    }
  });
  ipcMain.handle("get-supplier-remito-details", async (_event, remitoId) => {
    try {
      const remito = await db.select().from(supplierRemitos).where(eq(supplierRemitos.id, remitoId)).get();
      if (!remito) throw new Error("Remito not found");
      const items = await db.select({
        productId: supplierRemitoItems.productId,
        productName: products.name,
        productPresentation: products.presentation,
        quantity: supplierRemitoItems.quantity
      }).from(supplierRemitoItems).leftJoin(products, eq(supplierRemitoItems.productId, products.id)).where(eq(supplierRemitoItems.remitoId, remitoId));
      return { ...remito, items };
    } catch (error2) {
      console.error(error2);
      throw error2;
  ...
  ipcMain.handle("get-product-history", async (_event, productId) => {
    try {
      const result = await db.select().from(stockMovements).where(eq(stockMovements.productId, productId)).orderBy(sql`${stockMovements.date} DESC`);
      return result;
    } catch (error2) {
      console.error(error2);
      throw error2;
