...
function setupIpcHandlers(db, save) {
  ...
  ipcMain.handle("add-stock-movement", async (event, { productId, quantity, description, type: type2 }) => {
    try {
  ...
  ipcMain.handle("get-orders", async () => {
    try {
      const result = await db.select().from(orders).orderBy(orders.date);
      return result;
    } catch (error2) {
  ...
  ipcMain.handle("update-lot", async (event, data) => {
    try {
